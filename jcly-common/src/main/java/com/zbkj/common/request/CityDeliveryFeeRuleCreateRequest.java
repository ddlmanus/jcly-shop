...
@ApiModel(value = "CityDeliveryFeeRuleCreateRequest", description = "创建配送费用规则请求参数")
public class CityDeliveryFeeRuleCreateRequest implements Serializable {

    private static final long serialVersionUID = 1L;

    @NotBlank(message = "规则名称不能为空")
    @ApiModelProperty(value = "规则名称", required = true, example = "默认距离费用规则")
    private String ruleName;

    @NotNull(message = "基础费用不能为空")
    @ApiModelProperty(value = "基础费用", required = true, example = "5.00")
    private BigDecimal baseFee;

    @NotNull(message = "基础距离不能为空")
    @ApiModelProperty(value = "基础距离（公里）", required = true, example = "3.00")
    private BigDecimal baseDistance;

    @NotNull(message = "每公里费用不能为空")
    @ApiModelProperty(value = "每公里费用", required = true, example = "2.00")
    ...
    @ApiModelProperty(value = "最低费用", required = true, example = "5.00")
