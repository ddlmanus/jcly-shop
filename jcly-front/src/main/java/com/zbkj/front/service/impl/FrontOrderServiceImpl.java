...
public class FrontOrderServiceImpl implements FrontOrderService {
    ...
    public PreOrderResponse loadPreOrder(String preOrderNo) {
        ...
        for (PreMerchantOrderVo merchantOrderVo : merchantOrderVoList) {
            ...
            // 设置店铺会员积分信息
            Member member = memberService.getByUidAndMerId(user.getId(), merchantOrderVo.getMerId());
            if (member != null) {
                infoResponse.setMemberIntegral(member.getIntegral());
                
                // 获取积分规则并构建规则说明
                MemberIntegralRule integralRule = memberIntegralRuleService.getByMerId(merchantOrderVo.getMerId());
                if (integralRule != null) {
                    StringBuilder ruleBuilder = new StringBuilder();
                    if (integralRule.getIntegralToMoney() != null) {
                        ruleBuilder.append(integralRule.getIntegralToMoney().intValue()).append("积分=1元");
                    }
                    if (integralRule.getIntegralDeductionLimit() != null && integralRule.getIntegralDeductionLimit() > 0) {
                        if (ruleBuilder.length() > 0) ruleBuilder.append("，");
                        ruleBuilder.append("最多抵扣").append(integralRule.getIntegralDeductionLimit()).append("%");
                    }
                    infoResponse.setMemberIntegralRule(ruleBuilder.toString());
                }
            }
    ...
    public OrderNoResponse createOrder(CreateOrderRequest orderRequest) {
        ...
        // 处理发票信息
        if (orderRequest.getNeedInvoice()) {
            if (ObjectUtil.isNull(orderRequest.getInvoiceInfo())) {
                throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "发票信息不能为空");
            }
            order.setNeedInvoice(true);
            order.setInvoiceStatus(1); // 已申请
        } else {
        ...
        for (PreMerchantOrderVo merchantOrderVo : merchantOrderVoList) {
            ...
            for (OrderMerchantRequest om : orderMerchantRequestList) {
                if (om.getMerId().equals(merchantOrderVo.getMerId())) {
                    ...
                    // 处理店铺会员积分抵扣
                    if (om.getIsUseMemberIntegral() != null && om.getIsUseMemberIntegral()) {
                        Member member = memberService.getByUidAndMerId(user.getId(), merchantOrderVo.getMerId());
                        if (member != null && member.getIntegral() > 0) {
                            MemberIntegralRule integralRule = memberIntegralRuleService.getByMerId(merchantOrderVo.getMerId());
                            if (integralRule != null && integralRule.getIntegralToMoney() != null) {
                                
                                // 计算该商户的商品总金额（扣除优惠后）
                                BigDecimal merchantOrderPrice = merchantOrderVo.getProTotalFee().subtract(merchantOrderVo.getSvipDiscountPrice()).subtract(merchantOrderVo.getMerCouponFee());
                                
                                // 计算最大可抵扣金额（按照抵扣比例限制）
                                BigDecimal maxDeductionPrice = merchantOrderPrice;
                                if (integralRule.getIntegralDeductionLimit() != null && integralRule.getIntegralDeductionLimit() > 0) {
                                    maxDeductionPrice = merchantOrderPrice.multiply(new BigDecimal(integralRule.getIntegralDeductionLimit())).divide(new BigDecimal("100"), 2, RoundingMode.DOWN);
                                }
                                
                                // 根据用户拥有的积分计算可抵扣金额
                                BigDecimal userIntegralCanDeduct = new BigDecimal(member.getIntegral()).divide(integralRule.getIntegralToMoney(), 2, RoundingMode.DOWN);
                                
                                // 取两者中的较小值作为实际抵扣金额
                                BigDecimal actualDeductionPrice = userIntegralCanDeduct.min(maxDeductionPrice);
                                
                                // 根据实际抵扣金额计算使用的积分数量
                                int useMemberIntegral = actualDeductionPrice.multiply(integralRule.getIntegralToMoney()).intValue();
                                
                                // 添加店铺会员积分抵扣到商户订单
                                merchantOrder.setMemberIntegralDeduction(actualDeductionPrice);
                                merchantOrder.setUsedMemberIntegral(useMemberIntegral);
                            }
                        }
                    }
            ...
            // 计算支付金额，包含店铺会员积分抵扣
            BigDecimal payPrice = merchantOrder.getTotalPrice()
                .subtract(merchantOrder.getSvipDiscountPrice())
                .subtract(merchantOrder.getCouponPrice())
                .subtract(merchantOrder.getIntegralPrice());
            
            // 如果有店铺会员积分抵扣，也要减去
            if (merchantOrder.getMemberIntegralDeduction() != null) {
                payPrice = payPrice.subtract(merchantOrder.getMemberIntegralDeduction());
            }
            
            merchantOrder.setPayPrice(payPrice);
        ...
        // 重新计算平台订单支付金额，包含店铺会员积分抵扣
        BigDecimal totalMemberIntegralDeduction = merchantOrderList.stream()
            .filter(mo -> mo.getMemberIntegralDeduction() != null)
            .map(MerchantOrder::getMemberIntegralDeduction)
            .reduce(BigDecimal.ZERO, BigDecimal::add);
        
        // 重新设置支付金额 = 总金额 - SVIP优惠 - 优惠券 - 平台积分 - 店铺会员积分
        order.setPayPrice(order.getTotalPrice().subtract(order.getSvipDiscountPrice()).subtract(order.getCouponPrice()).subtract(order.getIntegralPrice()).subtract(totalMemberIntegralDeduction));
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_CLOUD)) { // 云盘订单
                ...
                // 同步聚水潭ERP库存
                try {
                    ProductAttrValue productAttrValue = productAttrValueService.getById(skuRecord.getInt("attrValueId"));
                    Integer stock= productAttrValue.getStock()-skuRecord.getInt("num");
                    justuitanErpService.syncInventory(skuRecord.getInt("productId"), skuRecord.getInt("attrValueId"), stock, "subtract");
                } catch (Exception ex) {
                    logger.warn("同步聚水潭ERP库存失败,商品ID：{},SKU ID：{},数量：{},错误：{}", skuRecord.getInt("productId"), skuRecord.getInt("attrValueId"), skuRecord.getInt("num"), ex.getMessage());
                }
                // 云盘商品规格扣库存
...
//                payComponentProductService.operationStock(skuRecord.getInt("productId"), skuRecord.getInt("num"), Constants.OPERATION_TYPE_SUBTRACT);
            ...
            if (order.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_NORMAL)
                ...
                for (MyRecord skuRecord : skuRecordList) {
                    ...
                    // 同步聚水潭ERP库存
                    try {
                        //这里需要使用规格的总库存- skuRecord.getInt("num")
                        ProductAttrValue productAttrValue = productAttrValueService.getById(skuRecord.getInt("attrValueId"));
                       Integer stock= productAttrValue.getStock()-skuRecord.getInt("num");
                        justuitanErpService.syncInventory(skuRecord.getInt("productId"), skuRecord.getInt("attrValueId"),  stock, "subtract");
                    } catch (Exception ex) {
            ...
            if (orderRequest.getNeedInvoice()) {
                ...
                for (MerchantOrder merchantOrder : merchantOrderList) {
                    ...
                    BeanUtils.copyProperties(orderRequest.getInvoiceInfo(), invoice);
                    invoice.setInvoiceNo(invoiceService.generateInvoiceNo());
                    invoice.setOrderNo(order.getOrderNo());
                    invoice.setUid(user.getId());
                    invoice.setMerId(merchantOrder.getMerId()); // 使用商户订单的商户ID
                    invoice.setInvoiceAmount(merchantOrder.getPayPrice()); // 使用商户订单的支付金额
                    invoice.setStatus(0); // 待开票
                    invoice.setAuditStatus(0); // 待审核
                    invoice.setIsDel(false);
                    invoice.setCreateTime(new Date());
                    invoice.setUpdateTime(new Date());
                    invoiceService.save(invoice);
                    invoiceIds.add(invoice.getId());
                }
                // 更新订单表中的发票ID（使用第一张发票的ID，或者可以用逗号分隔的ID列表）
                if (!invoiceIds.isEmpty()) {
                    Order updateOrder = new Order();
                    updateOrder.setId(order.getId());
                    updateOrder.setInvoiceId(invoiceIds.get(0)); // 主要用于兼容性
        ...
        // 订单创建成功后上传到聚水潭
        try {
            justuitanErpService.uploadOrderToJst(order);
            logger.info("订单上传聚水潭成功，订单号：{}", order.getOrderNo());
        } catch (Exception ex) {
            logger.warn("订单上传聚水潭失败，订单号：{}，错误：{}", order.getOrderNo(), ex.getMessage());
        }
        // 生成并上传订单Markdown文件到Coze知识库
            try {
                orderMarkdownService.generateAndUploadOrderMarkdown(order);
            } catch (Exception ex) {
                logger.error("生成上传订单Markdown文件失败，订单号：{}", order.getOrderNo(), ex);
                // 这里不影响主流程，只记录错误日志
            }
    ...
    public PageInfo<OrderFrontDataResponse> list_v1_4(OrderFrontListRequest request) {
        ...
        for (Order order : orderList) {
            ...
            //查询发票信息
            infoResponse.setInvoice(invoiceService.getByOrderNo(order.getOrderNo()));
    ...
    @Override
    public UserIntegralReponse userCenterIntegralNum() {
        Integer userId = userService.getUserId();
        LambdaQueryWrapper<Order> lambda = new LambdaQueryWrapper<>();
        lambda.eq(Order::getUid, userId);
        lambda.eq(Order::getPaid, true);
        lambda.eq(Order::getIsDel, false);
        lambda.eq(Order::getStatus, 6);
        List<Order> orders = orderService.selectList(lambda);
        //获取缓存中的积分
        Integer oldIntegral = redisUtil.get("user:integral:" + userId);
        //如果缓存不存在
        if (oldIntegral == null) {
            // 获取用户总积分
            Integer integral = orders.stream().mapToInt(Order::getIntegral).sum()==0?0:orders.stream().mapToInt(Order::getIntegral).sum();
            redisUtil.set("user:integral:" + userId, integral);
        }

        // 获取用户总砖石
        Integer diamond = orders.stream().mapToInt(Order::getDiamond).sum();
        //需要计算出这次查询比上一次查询新增了多少积分
        Integer integral = orders.stream().mapToInt(Order::getIntegral).sum();
       Integer addIntegral = integral - oldIntegral;
        UserIntegralReponse userIntegralReponse = new UserIntegralReponse();
        userIntegralReponse.setIntegralCount(integral==null ? 0 : integral);
        userIntegralReponse.setIntegralNewCount(addIntegral==null ? 0 : addIntegral);
        return userIntegralReponse;
    }

    @Override
    public LogisticsResultVo getLogisticsInfoByOrderNo(String orderNO) {
        return orderService.getLogisticsInfoByOrderNo(orderNO);
    }
...
//        }
        ...
        //获取发票信息
        response.setInvoice(invoiceService.getByOrderNo(orderNo));
    ...
    public Boolean cancel(String orderNo) {
        ...
        if (cancel) {
            ...
            // 调用聚水潭订单取消接口
            try {
                justuitanErpService.cancelOrderInJst(order.getOrderNo(), "用户取消", "用户主动取消订单");
            } catch (Exception e) {
                logger.error("聚水潭订单取消失败，订单号：{}，错误信息：{}", order.getOrderNo(), e.getMessage(), e);
            }
        }
        // 处理拼团的取消订单逻辑
        if(order.getType().equals(OrderConstants.ORDER_TYPE_PITUAN)){
    ...
    public Boolean refundApply(OrderRefundApplyRequest request) {
        ...
        if (execute) {
            try {
                createRefundTodoForMerchant(order.getMerId(), refundOrder.getRefundOrderNo(), order.getOrderNo(), userService.getInfo());
            } catch (Exception ex) {
                logger.error("创建退款待办事项失败，商户ID:{}, 退款单号:{}, 错误信息:{}", 
                    order.getMerId(), refundOrder.getRefundOrderNo(), ex.getMessage());
            }
        }
        
        // 调用聚水潭售后上传接口
        try {
            justuitanErpService.uploadAfterSaleToJst(
                order.getOrderNo(),
                refundOrder.getRefundOrderNo(),
                String.valueOf(request.getAfterSalesType()),
                refundOrder.getRefundPrice(),
                refundOrder.getRefundReasonWap(),
                "WAIT_SELLER_AGREE"  // 用户申请退款，设置状态为等待商家同意
            );
        } catch (Exception e) {
            logger.error("聚水潭售后上传失败，订单号：{}，售后单号：{}，错误信息：{}", order.getOrderNo(), refundOrder.getRefundOrderNo(), e.getMessage(), e);
        }
            ...
            for (Map.Entry<Integer, MyRecord> m : tempMap.entrySet()) {
                ...
                switch (shippingTemplate.getType()) {
                    case ShippingTemplatesConstants.CHARGE_MODE_TYPE_NUMBER: // 件数
                        ...
                        List<PreOrderInfoDetailVo> detailVoList = merchantOrderVo.getOrderInfoList().stream().filter(e -> e.getTempId().equals(m.getKey())).collect(Collectors.toList());
...
//                break;
    ...
    private ComputedOrderPriceResponse computedPrice(OrderComputedPriceRequest request, PreOrderInfoVo orderInfoVo, User user) {
        ...
        if (orderInfoVo.getType().equals(OrderConstants.ORDER_TYPE_SECKILL) || orderInfoVo.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_VIDEO)
                || orderInfoVo.getSecondType().equals(OrderConstants.ORDER_SECOND_TYPE_INTEGRAL)) {
            priceResponse.setMerOrderResponseList(computedPriceGetMerOrderList(orderInfoVo, false, request));
        } else {
            getCouponFee_V1_3(orderInfoVo, user.getId(), user.getIsPaidMember());
            ...
            priceResponse.setMerOrderResponseList(computedPriceGetMerOrderList(orderInfoVo, user.getIsPaidMember(), request));
        ...
        // 计算店铺会员积分总抵扣金额
        BigDecimal totalMemberIntegralDeduction = BigDecimal.ZERO;
        if (priceResponse.getMerOrderResponseList() != null) {
            totalMemberIntegralDeduction = priceResponse.getMerOrderResponseList().stream()
                .map(ComputedMerchantOrderResponse::getMemberIntegralDeductionPrice)
                .filter(price -> price != null)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        }
        
        // 从支付金额中减去店铺会员积分抵扣
        payPrice = payPrice.subtract(totalMemberIntegralDeduction);
    ...
    private List<ComputedMerchantOrderResponse> computedPriceGetMerOrderList(PreOrderInfoVo orderInfoVo, Boolean userIsPaidMember, OrderComputedPriceRequest request) {
        Integer userId = userService.getUserId();
        List<ComputedMerchantOrderResponse> merOrderResponseList = orderInfoVo.getMerchantOrderVoList().stream().map(vo -> {
            ...
            // 处理店铺会员积分
            Member member = memberService.getByUidAndMerId(userId, vo.getMerId());
            if (member != null) {
                merOrderResponse.setMemberIntegral(member.getIntegral());
                // 从请求中获取用户是否选择使用店铺会员积分
                OrderMerchantRequest merchantRequest = request.getOrderMerchantRequestList().stream()
                    .filter(m -> m.getMerId().equals(vo.getMerId()))
                    .findFirst().orElse(null);
                
                // 只有当用户选择使用店铺会员积分且有可用积分时才计算
                if (merchantRequest != null && merchantRequest.getIsUseMemberIntegral() != null 
                    && merchantRequest.getIsUseMemberIntegral() && member.getIntegral() > 0) {
                    
                    MemberIntegralRule integralRule = memberIntegralRuleService.getByMerId(vo.getMerId());
                    if (integralRule != null && integralRule.getIntegralToMoney() != null) {
                        
                        // 计算该商户的商品总金额（扣除优惠后）
                        BigDecimal merchantOrderPrice = vo.getProTotalFee().subtract(vo.getSvipDiscountPrice()).subtract(vo.getMerCouponFee());
                        ...
                        // 设置返回值
                        merOrderResponse.setMemberIntegralDeductionPrice(actualDeductionPrice);
                        merOrderResponse.setUsedMemberIntegral(useMemberIntegral);
                        merOrderResponse.setSurplusMemberIntegral(member.getIntegral() - useMemberIntegral);
                        merOrderResponse.setIsUseMemberIntegral(true);
                    }
                } else {
                    merOrderResponse.setMemberIntegralDeductionPrice(BigDecimal.ZERO);
                    merOrderResponse.setUsedMemberIntegral(0);
                    merOrderResponse.setSurplusMemberIntegral(member.getIntegral());
                    merOrderResponse.setIsUseMemberIntegral(false);
                }
            } else {
                // 如果没有会员信息，设置默认值
                merOrderResponse.setMemberIntegral(0);
                merOrderResponse.setMemberIntegralDeductionPrice(BigDecimal.ZERO);
                merOrderResponse.setUsedMemberIntegral(0);
                merOrderResponse.setSurplusMemberIntegral(0);
                merOrderResponse.setIsUseMemberIntegral(false);
            }
            // 查询适用的用户优惠券
...
//                    throw new CrmebException("总金额小于优惠券最小使用金额");
     ...
     * @param user          申请用户信息
     */
    private void createRefundTodoForMerchant(Integer merId, String refundOrderNo, String orderNo, User user) {
        try {
            MerchantTodoItem todoItem = new MerchantTodoItem();
            todoItem.setMerId(merId);
            todoItem.setType("refund");
            todoItem.setTitle("新退款申请待处理");
            todoItem.setContent(String.format("用户：%s 申请退款，退款单号：%s，原订单号：%s，请及时处理！", 
                user.getNickname(), refundOrderNo, orderNo));
            todoItem.setPriority(3); // 高优先级
            todoItem.setRelatedId(refundOrderNo);
            todoItem.setTargetUrl("/merchant/refund/detail/" + refundOrderNo);
            
            Boolean result = merchantTodoItemService.create(todoItem);
            if (result) {
                logger.info("商户退款待办事项创建成功，商户ID:{}, 退款单号:{}", merId, refundOrderNo);
            } else {
                logger.error("商户退款待办事项创建失败，商户ID:{}, 退款单号:{}", merId, refundOrderNo);
            }
        } catch (Exception e) {
            logger.error("创建商户退款待办事项异常，商户ID:{}, 退款单号:{}, 错误信息:{}", 
                merId, refundOrderNo, e.getMessage(), e);
        }
    }
}
