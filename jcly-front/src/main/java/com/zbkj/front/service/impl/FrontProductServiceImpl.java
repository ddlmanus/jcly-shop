...
//            return componentProductService.getH5Detail(id);
    ...
    private ProductDetailResponse getBaseDetail(Integer id) {
        ...
        // 获取商品评价数据
        PageParamRequest replyPageRequest = new PageParamRequest();
        replyPageRequest.setPage(1);
        replyPageRequest.setLimit(10); // 默认返回10条评价
        PageInfo<ProductReplyResponse> replyPageInfo = productReplyService.getH5List(product.getId(), 0, replyPageRequest);
        productDetailResponse.setReplyList(replyPageInfo.getList());
    ...
    public List<ProductFrontResponse> getProductByIds(List<Integer> proIdList) {
        ...
        productBrandService.findByIdList(byIdsAndLabel.stream().map(Product::getBrandId).collect(Collectors.toList())).forEach(productBrand -> {
            productFrontResponses.forEach(productFrontResponse -> {
                if (productFrontResponse.getBrandId().equals(productBrand.getId())) {
                    productFrontResponse.setBrandName(productBrand.getName());
                }
            });
        });
    ...
    @Override
    public List<ProductFrontResponse> searchList(ProductFrontSearchRequest request, PageParamRequest pageRequest) {
        List<ProductFrontResponse> responseList = new ArrayList<>();

        // 使用request中的query字段作为关键词列表
        ...
        if(StringUtils.isNotBlank( keyword)){
            ...
            for (String kw : split) {
                ...
                PageInfo<ProductFrontResponse> h5List = productService.findH5List(request, pageRequest);
                responseList.addAll(h5List.getList());
            }
        }else{
            pageRequest.setLimit(3);
            pageRequest.setPage(1);
