...
@Slf4j
@RestController
@RequestMapping("/api/admin/platform/webrtc")
@Api(tags = "平台端 - WebRTC音频通话管理")
public class PlatformWebRTCController {
    ...
    public CommonResult<Map<String, Object>> staffCallUser(@RequestBody Map<String, Object> callRequest) {
        try {
            ...
            // 客服呼叫用户
            Map<String, Object> result = webRTCService.initiateCall(staffId, userId, "PLATFORM", "USER");
            result.put("sessionId", sessionId);
            
            log.info("平台客服{}呼叫用户{}: callId={}", staffId, userId, result.get("callId"));
            return CommonResult.success(result);
        } catch (Exception e) {
            log.error("平台客服呼叫用户失败", e);
