...
@Slf4j
@RestController
@RequestMapping("api/admin/merchant/city-delivery/map")
@Api(tags = "同城配送腾讯地图服务控制器")
@Validated
public class CityDeliveryMapController {

    @Autowired
    private TencentMapService tencentMapService;

   // @PreAuthorize("hasAuthority('merchant:city:delivery:map:geocoding')")
 ...
 //   @PreAuthorize("hasAuthority('merchant:city:delivery:map:reverse')")
 ...
 //   @PreAuthorize("hasAuthority('merchant:city:delivery:map:distance')")
   ...
   // @LogControllerAnnotation(intoDB = true, methodType = MethodType.OTHER, description = "地址智能提示")
 //   @PreAuthorize("hasAuthority('merchant:city:delivery:map:suggestion')")
