...
@Slf4j
@RestController
@RequestMapping("api/admin/merchant/unified-chat")
@Api(tags = "商户端 - 统一聊天管理")
public class UnifiedChatController {
    ...
    public CommonResult<Integer> getUserId() {
        ...
        return CommonResult.success(loginUser.getUser().getId());
    }
    /**
     * 创建或获取聊天会话
     */
    ...
    public CommonResult<UnifiedChatSession> createOrGetSessionByContact(
        ...
        try {
            // 获取当前登录的管理员信息
            com.zbkj.common.vo.LoginUserVo loginUser = com.zbkj.common.utils.SecurityUtil.getLoginUserVo();
            Long merId = loginUser.getUser().getMerId().longValue(); // 商户ID
    ...
    public CommonResult<MessageResponse> sendMessageToContact(
        ...
        try {
            ...
            if(merId.intValue()!=0){
                request.setSenderType(UnifiedChatSession.USER_TYPE_MERCHANT); // 发送者是平台管理员
            }else{
                request.setSenderType(UnifiedChatSession.USER_TYPE_PLATFORM); // 发送者是商户管理员
            }
