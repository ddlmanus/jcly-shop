        ...
        if (execute) {
            try {
                ...
                if (StrUtil.isNotBlank(knowledgeId)) {
                    ...
                    new Thread(() -> {
                        try {
                            knowledgeMarkdownService.generateAndUploadSeckillMarkdown(
                                activity.getId(), knowledgeId, 0); // 平台创建的秒杀活动，merchantId设为0
                            logger.info("秒杀活动知识库上传成功，活动ID: {}", activity.getId());
                        } catch (Exception ex) {
                            logger.error("秒杀活动知识库上传失败，活动ID: {}", activity.getId(), ex);
                        }
                    }).start();
                } else {
                    logger.warn("未配置Coze知识库ID，跳过秒杀活动知识库上传");
                }
            } catch (Exception ex) {
                logger.error("启动秒杀活动知识库上传任务失败", ex);
            }
        }
