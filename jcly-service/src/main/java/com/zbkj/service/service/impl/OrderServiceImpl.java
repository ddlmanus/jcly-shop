...
public class OrderServiceImpl extends ServiceImpl<OrderDao, Order> implements OrderService {
    ...
    @Autowired
    private OrderMarkdownService orderMarkdownService;
    ...
    public Boolean updatePaid(String orderNo) {
        ...
        Boolean result = update(lqw);
        
        // 如果更新成功，发布订单支付事件触发WebSocket推送
        if (result) {
            try {
                // 更新订单对象的支付状态
                order.setPaid(true);
                ...
                // 发布订单支付事件
                dataChangeEventPublisher.publishOrderPaid(order);
                logger.info("订单支付成功，已触发WebSocket推送: orderNo={}", orderNo);
                
                // 同步更新到Coze知识库
                try {
                        try {
                            String newCozeFileId = orderMarkdownService.updateOrderMarkdownInKnowledge(order);
                            if (StrUtil.isNotBlank(newCozeFileId)) {
                                logger.info("订单支付成功后Coze知识库同步成功，订单号：{}，文件ID：{}", 
                                    order.getOrderNo(), newCozeFileId);
                            } else {
                                logger.warn("订单支付成功后Coze知识库同步失败，订单号：{}", order.getOrderNo());
                            }
                        } catch (Exception ex) {
                            logger.error("订单支付成功后Coze知识库同步异常，订单号：{}", order.getOrderNo(), ex);
                        }
                } catch (Exception e) {
                    logger.error("启动订单支付成功后知识库同步任务失败，订单号：{}", order.getOrderNo(), e);
                }
            } catch (Exception e) {
                logger.error("发布订单支付事件失败: orderNo={}", orderNo, e);
                // 不影响支付流程，只记录错误
            }
        }
    ...
    /**
     * 更新订单并同步到Coze知识库
     * 当订单状态发生重要变更时（支付、发货、完成等），同步更新知识库中的订单信息
     * 
     * @param order 要更新的订单对象
     * @param syncToKnowledge 是否同步到知识库，默认true
     * @return 更新结果
     */
    public Boolean updateByIdAndSyncToKnowledge(Order order, Boolean syncToKnowledge) {
        if (order == null) {
            return false;
        }
        ...
        Order oldOrder = null;
        if (order.getId() != null) {
            oldOrder = getById(order.getId());
        }
        
        // 更新订单
        Boolean updateResult = updateById(order);
        
        if (updateResult && syncToKnowledge != null && syncToKnowledge) {
            // 检查是否是需要同步的状态变更
            boolean shouldSync = shouldSyncToKnowledge(oldOrder, order);
            
            if (shouldSync) {
                // 异步同步到知识库，不影响主业务流程
                try {
                    logger.info("订单状态发生变更，开始同步到Coze知识库，订单号：{}，旧状态：{}，新状态：{}", 
                        order.getOrderNo(), 
                        oldOrder != null ? oldOrder.getStatus() : "null", 
                        order.getStatus());
                    ...
                    // 使用异步服务执行同步操作
                        try {
                            String newCozeFileId = orderMarkdownService.updateOrderMarkdownInKnowledge(order);
                            if (StrUtil.isNotBlank(newCozeFileId)) {
                                logger.info("订单Coze知识库同步成功，订单号：{}，文件ID：{}", 
                                    order.getOrderNo(), newCozeFileId);
                            } else {
                                logger.warn("订单Coze知识库同步失败，订单号：{}", order.getOrderNo());
                            }
                        } catch (Exception ex) {
                            logger.error("订单Coze知识库同步异常，订单号：{}", order.getOrderNo(), ex);
                        }
                } catch (Exception e) {
                    logger.error("启动订单知识库同步任务失败，订单号：{}", order.getOrderNo(), e);
                }
            }
        }
    ...
    /**
     * 更新订单并同步到Coze知识库（默认同步）
     */
    public Boolean updateByIdAndSyncToKnowledge(Order order) {
        return updateByIdAndSyncToKnowledge(order, true);
    }
    
    /**
     * 判断是否需要同步到知识库
     * 以下情况会触发同步：
     * 1. 订单状态发生变更（支付、发货、完成、取消等）
     * 2. 支付状态发生变更
     * 3. 退款状态发生变更
     */
    private boolean shouldSyncToKnowledge(Order oldOrder, Order newOrder) {
        if (oldOrder == null || newOrder == null) {
            return true; // 新订单或无法获取旧订单信息时同步
        }
        
        // 订单状态发生变更
        if (!Objects.equals(oldOrder.getStatus(), newOrder.getStatus())) {
            logger.debug("订单状态变更：{} -> {}，需要同步到知识库", 
                oldOrder.getStatus(), newOrder.getStatus());
            return true;
        }
        
        // 支付状态发生变更
        if (!Objects.equals(oldOrder.getPaid(), newOrder.getPaid())) {
            logger.debug("订单支付状态变更：{} -> {}，需要同步到知识库", 
                oldOrder.getPaid(), newOrder.getPaid());
            return true;
        }
        
        // 退款状态发生变更
        if (!Objects.equals(oldOrder.getRefundStatus(), newOrder.getRefundStatus())) {
            logger.debug("订单退款状态变更：{} -> {}，需要同步到知识库", 
                oldOrder.getRefundStatus(), newOrder.getRefundStatus());
            return true;
        }
    ...
    public PageInfo<MerchantOrderPageResponse> getMerchantAdminPage(OrderSearchRequest request, SystemAdmin systemAdmin) {
        ...
        responseList.forEach(response -> {
            ...
            // 填充 productList
            List<OrderProductResponse> productList = orderDetailList.stream().map(detail -> {
                OrderProductResponse productResponse = new OrderProductResponse();
                productResponse.setId(detail.getId());
                productResponse.setOrderId(detail.getId());
                productResponse.setProductId(detail.getProductId());
                productResponse.setProductName(detail.getProductName());
                productResponse.setImage(detail.getImage());
                productResponse.setSku(detail.getSku());
                productResponse.setPrice(detail.getPrice());
                productResponse.setNum(detail.getPayNum());
                productResponse.setTotalPrice(detail.getPayPrice().multiply(new BigDecimal(detail.getPayNum())));
     ...
     */
    @Override
    public OrderCountItemResponse getMerchantOrderStatusNum(OrderSearchRequest request, SystemAdmin systemAdmin) {
    ...
    public Boolean send(OrderSendRequest request, SystemAdmin systemAdmin) {
        ...
        if (!request.getIsSplit()) {// 全部发货
            ...
            if (request.getDeliveryType().equals(OrderConstants.ORDER_DELIVERY_TYPE_CITY_DELIVERY)) {
                return sendCityDelivery(request, order, merchantOrder);
            }
        }
        ...
        if (request.getDeliveryType().equals(OrderConstants.ORDER_DELIVERY_TYPE_CITY_DELIVERY)) {
            return splitSendCityDelivery(request, order, merchantOrder, orderDetailList);
        }
        return Boolean.TRUE;
    }
    @Override
    ...
    private Boolean splitSendNoNeed(OrderSendRequest request, Order order, MerchantOrder merchantOrder, List<OrderDetail> orderDetailList) {
        ...
        Boolean execute = transactionTemplate.execute(i -> {
            updateByIdAndSyncToKnowledge(order);
        ...
        // 发布订单更新事件，触发WebSocket推送
        try {
            Map<String, Object> changeFields = new HashMap<>();
            changeFields.put("status", OrderConstants.ORDER_STATUS_PART_SHIPPING);
            changeFields.put("deliveryType", "NO_NEED");
            dataChangeEventPublisher.publishOrderUpdated(order, changeFields);
            logger.info("订单发货成功，已触发WebSocket推送: orderNo={}, status={}", order.getOrderNo(), order.getStatus());
        } catch (Exception e) {
            logger.error("发布订单更新事件失败: orderNo={}", order.getOrderNo(), e);
            // 不影响发货流程，只记录错误
        }
        ...
        if (count <= 0) {
            ...
            // 发布订单状态变为待收货的事件
            try {
                Map<String, Object> receiptChangeFields = new HashMap<>();
                receiptChangeFields.put("status", OrderConstants.ORDER_STATUS_WAIT_RECEIPT);
                dataChangeEventPublisher.publishOrderUpdated(order, receiptChangeFields);
                logger.info("订单全部发货完成，已触发WebSocket推送: orderNo={}, status={}", order.getOrderNo(), order.getStatus());
            } catch (Exception e) {
                logger.error("发布订单更新事件失败: orderNo={}", order.getOrderNo(), e);
            }
        ...
        // 聚水潭无需发货拆单发货接口调用
        try {
            justuitanErpService.shipOrderToJst(order.getOrderNo(), "无需发货", "");
            logger.info("聚水潭无需发货拆单发货接口调用成功，订单号：{}", order.getOrderNo());
        } catch (Exception e) {
            logger.error("聚水潭无需发货拆单发货接口调用失败，订单号：{}，错误信息：{}", order.getOrderNo(), e.getMessage(), e);
        }
    ...
    private Boolean splitSendMerchant(OrderSendRequest request, Order order, MerchantOrder merchantOrder, List<OrderDetail> orderDetailList) {
        ...
        // 聚水潭商家配送拆单发货接口调用
        try {
            justuitanErpService.shipOrderToJst(order.getOrderNo(), "商家配送", "");
            logger.info("聚水潭商家配送拆单发货接口调用成功，订单号：{}", order.getOrderNo());
        } catch (Exception e) {
            logger.error("聚水潭商家配送拆单发货接口调用失败，订单号：{}，错误信息：{}", order.getOrderNo(), e.getMessage(), e);
        }
    ...
    private Boolean sendNoNeed(String deliveryMark, Order order, MerchantOrder merchantOrder) {
        ...
        try {
            justuitanErpService.shipOrderToJst(order.getOrderNo(), "无需发货", "");
            logger.info("聚水潭无需发货接口调用成功，订单号：{}", order.getOrderNo());
        } catch (Exception e) {
            logger.error("聚水潭无需发货接口调用失败，订单号：{}，错误信息：{}", order.getOrderNo(), e.getMessage(), e);
        }
    ...
    private Boolean sendMerchant(String deliveryCarrier, String carrierPhone, Order order, MerchantOrder merchantOrder) {
        ...
        try {
            justuitanErpService.shipOrderToJst(order.getOrderNo(), "商家配送", "");
            logger.info("聚水潭商家配送发货接口调用成功，订单号：{}", order.getOrderNo());
        } catch (Exception e) {
            logger.error("聚水潭商家配送发货接口调用失败，订单号：{}，错误信息：{}", order.getOrderNo(), e.getMessage(), e);
        }
     ...
     */
    private Boolean sendCityDelivery(OrderSendRequest request, Order order, MerchantOrder merchantOrder) {
        // 参数验证
        if (StrUtil.isBlank(request.getCityDeliveryDriverName())) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "配送员姓名不能为空");
        }
        if (StrUtil.isBlank(request.getCityDeliveryDriverPhone())) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "配送员手机号不能为空");
        }
        if (StrUtil.isBlank(request.getCityDeliveryPickupAddress())) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "取件地址不能为空");
        }
        if (StrUtil.isBlank(request.getCityDeliveryDeliveryAddress())) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "送货地址不能为空");
        }
        ...
        // 创建同城配送订单
        try {
            BigDecimal deliveryFee = request.getCityDeliveryFee() != null ? request.getCityDeliveryFee() : BigDecimal.ZERO;
            BigDecimal deliveryDistance = request.getCityDeliveryDistance() != null ? request.getCityDeliveryDistance() : BigDecimal.ZERO;
            Integer deliveryType = request.getCityDeliveryType() != null ? request.getCityDeliveryType() : 1; // 默认即时配送
            
            // 获取商户信息
            Merchant merchant = merchantService.getById(order.getMerId());
            String pickupContact = merchant != null ? merchant.getName() : "商户";
            String pickupPhone = merchant != null ? merchant.getPhone() : "";
            User user = userService.getById(order.getUid());
            CityDeliveryOrder deliveryOrder = cityDeliveryService.createDeliveryOrder(
                order.getOrderNo(),
                order.getMerId(),
                order.getUid(),
                request.getCityDeliveryPickupAddress(),
                pickupContact, // 使用商户名称作为取件联系人
                pickupPhone, // 使用商户手机号作为取件联系电话
                request.getCityDeliveryDeliveryAddress(),
                user.getRealName(), // 使用订单收货人
                user.getPhone(), // 使用订单收货人手机号
                deliveryType,
                request.getCityDeliveryEstimatedTime(),
                deliveryFee,
                deliveryDistance,
                request.getCityDeliveryRemark()
            );

            // 分配配送员
            cityDeliveryService.assignDriver(
                deliveryOrder.getDeliveryOrderNo(),
                null, // 暂时不指定配送员ID
                request.getCityDeliveryDriverName(),
                request.getCityDeliveryDriverPhone()
            );
            ...
            invoice.setOrderNo(order.getOrderNo());
            invoice.setUid(order.getUid());
            invoice.setExpressCode("CITY_DELIVERY"); // 同城配送标识
            invoice.setExpressName("同城配送");
            invoice.setTrackingNumber(deliveryOrder.getDeliveryOrderNo()); // 使用配送订单号作为跟踪号
            invoice.setExpressRecordType("3"); // 同城配送类型
            invoice.setTotalNum(orderInvoiceDetailList.stream().mapToInt(OrderInvoiceDetail::getNum).sum());

            merchantOrder.setIsSplitDelivery(Boolean.FALSE);
            merchantOrder.setDeliveryType(OrderConstants.ORDER_DELIVERY_TYPE_CITY_DELIVERY);
            merchantOrder.setUpdateTime(DateUtil.date());
            order.setStatus(OrderConstants.ORDER_STATUS_WAIT_RECEIPT);
            order.setUpdateTime(DateUtil.date());

            String message = StrUtil.format(OrderStatusConstants.ORDER_LOG_MESSAGE_SEND_CITY_DELIVERY,
                    request.getCityDeliveryDriverName(),
                    request.getCityDeliveryDriverPhone(),
                    request.getCityDeliveryEstimatedTime() != null ? request.getCityDeliveryEstimatedTime() : "2小时内");
            ...
            if (!execute) throw new CrmebException("同城配送发货失败！");

            // 发送通知消息（可选）
            sendCityDeliveryNotification(order, deliveryOrder, orderInvoiceDetailList);
            
            // 聚水潭发货接口调用
            try {
                justuitanErpService.shipOrderToJst(order.getOrderNo(), "同城配送", "");
                logger.info("聚水潭同城配送发货接口调用成功，订单号：{}", order.getOrderNo());
            } catch (Exception e) {
                logger.error("聚水潭同城配送发货接口调用失败，订单号：{}，错误信息：{}", order.getOrderNo(), e.getMessage(), e);
            }

            return execute;
        } catch (Exception e) {
            logger.error("同城配送发货失败：{}", e.getMessage(), e);
            throw new CrmebException("同城配送发货失败：" + e.getMessage());
        }
    }
     ...
     */
    private Boolean splitSendCityDelivery(OrderSendRequest request, Order order, MerchantOrder merchantOrder, List<OrderDetail> orderDetailList) {
        // 参数验证
        if (StrUtil.isBlank(request.getCityDeliveryDriverName())) {
            throw new CrmebException(CommonResultCode.VALIDATE_FAILED, "配送员姓名不能为空");
        }
        ...
        detailRequestList.forEach(detail -> {
            OrderDetail orderDetail = orderDetailList.stream()
                    .filter(e -> e.getId().equals(detail.getOrderDetailId())).findAny().orElse(null);
            if (orderDetail != null) {
                orderDetail.setDeliveryNum(orderDetail.getDeliveryNum() + detail.getNum());
                orderDetail.setUpdateTime(DateUtil.date());
                OrderInvoiceDetail invoiceDetail = new OrderInvoiceDetail();
        ...
        try {
            ...
            CityDeliveryOrder deliveryOrder = cityDeliveryService.createDeliveryOrder(
                ...
                pickupPhone, // 使用商户手机号作为取件联系电话
                request.getCityDeliveryDeliveryAddress(),
                merchantOrder.getRealName(),
                merchantOrder.getUserPhone(),
                deliveryType,
                request.getCityDeliveryEstimatedTime(),
                deliveryFee,
                deliveryDistance,
            ...
            invoice.setOrderNo(order.getOrderNo());
            invoice.setUid(order.getUid());
            invoice.setExpressCode("CITY_DELIVERY");
            invoice.setExpressName("同城配送");
            invoice.setTrackingNumber(deliveryOrder.getDeliveryOrderNo());
            invoice.setExpressRecordType("3");
            invoice.setTotalNum(orderInvoiceDetailList.stream().mapToInt(OrderInvoiceDetail::getNum).sum());
            ...
            if (!execute) throw new CrmebException("同城配送拆单发货失败！");
            ...
            try {
                justuitanErpService.shipOrderToJst(order.getOrderNo(), "同城配送", "");
                logger.info("聚水潭同城配送拆单发货接口调用成功，订单号：{}", order.getOrderNo());
            } catch (Exception e) {
                logger.error("聚水潭同城配送拆单发货接口调用失败，订单号：{}，错误信息：{}", order.getOrderNo(), e.getMessage(), e);
            }

            return execute;
        } catch (Exception e) {
            logger.error("同城配送拆单发货失败：{}", e.getMessage(), e);
            throw new CrmebException("同城配送拆单发货失败：" + e.getMessage());
        }
    }
     ...
     * 发送同城配送通知
     */
    private void sendCityDeliveryNotification(Order order, CityDeliveryOrder deliveryOrder, List<OrderInvoiceDetail> orderInvoiceDetailList) {
        try {
            // 获取通知配置
            ...
            if (payNotification.getIsSms().equals(1)) {
                ...
                if (StrUtil.isNotBlank(user.getPhone())) {
                    try {
                        List<String> productNameList = orderInvoiceDetailList.stream()
                                .map(OrderInvoiceDetail::getProductName).collect(Collectors.toList());
                        String storeNameAndCarNumString = String.join(",", productNameList);
                        
                        // 发送同城配送短信通知
                        smsService.sendOrderDeliverNotice(user.getPhone(), user.getNickname(), 
                                storeNameAndCarNumString, order.getOrderNo());
                    } catch (Exception e) {
                        logger.error("同城配送短信通知发送异常", e);
                    }
                }
            }
            ...
            if (payNotification.getIsWechat().equals(1) || payNotification.getIsRoutine().equals(1)) {
                try {
                    // 这里可以发送微信模板消息，告知用户同城配送信息
                    logger.info("同城配送微信通知：订单号[{}], 配送单号[{}]", order.getOrderNo(), deliveryOrder.getDeliveryOrderNo());
                } catch (Exception e) {
                    logger.error("同城配送微信通知发送失败", e);
                }
            }
        } catch (Exception e) {
            logger.error("同城配送通知发送异常：{}", e.getMessage(), e);
        }
    }
    ...
    public Boolean merchantMark(OrderRemarkRequest request, SystemAdmin systemAdmin) {
        ...
        Boolean result = merchantOrderService.updateById(merchantOrder);
        
        // 同步订单备注到聚水潭（仅自营店）
        if (result && StrUtil.isNotBlank(order.getJstOrderId())) {
            try {
                justuitanErpService.updateOrderRemarkInJst(order.getJstOrderId(), request.getRemark(), false);
                logger.info("同步订单备注到聚水潭成功，订单号: {}", order.getOrderNo());
            } catch (Exception e) {
                logger.error("同步订单备注到聚水潭失败，订单号: {}", order.getOrderNo(), e);
                // 不影响主要业务流程，只记录错误日志
            }
        }
    ...
    public Boolean takeDelivery(String orderNo) {
        ...
        // 确认收货成功后，同步更新聚水潭订单状态为交易成功
        if (result) {
            try {
                Order order = getByOrderNo(orderNo);
                if (order != null && justuitanErpService.isSelfOperatedStore(order.getMerId())) {
                    logger.info("用户确认收货，开始同步到聚水潭，订单号: {}", orderNo);
                    
                    // 构建临时订单对象，仅用于更新状态
                    Order tempOrder = new Order();
                    BeanUtils.copyProperties(order, tempOrder);
                    tempOrder.setStatus(OrderConstants.ORDER_STATUS_TAKE_DELIVERY); // 已完成状态
                    
                    // 调用聚水潭订单上传接口，更新订单状态为TRADE_FINISHED（交易成功）
                    JustuitanOrderUploadResult uploadResult = justuitanErpService.uploadOrderToJst(tempOrder);
                    if (uploadResult.isSuccess()) {
                        logger.info("成功将聚水潭订单状态更新为交易成功: orderNo={}", orderNo);
                    } else {
                        logger.error("更新聚水潭订单状态为交易成功失败: orderNo={}, 错误: {}", orderNo, uploadResult.getMessage());
                    }
                } else {
                    logger.debug("订单不属于自营店，跳过同步到聚水潭: orderNo={}", orderNo);
                }
            } catch (Exception e) {
                logger.error("同步聚水潭订单状态为交易成功异常: orderNo={}, 错误: {}", orderNo, e.getMessage(), e);
            }
        }
    ...
    public PageInfo<PlatformOrderPageResponse> getPlatformAdminPage(OrderSearchRequest request) {
        ...
        // 填充订单商品信息
        List<String> orderNoList = responseList.stream().map(PlatformOrderPageResponse::getOrderNo).collect(Collectors.toList());
        Map<String, List<OrderDetail>> orderDetailMap = orderDetailService.getMapByOrderNoList(orderNoList);
        responseList.forEach(response -> {
            List<OrderDetail> orderDetailList = orderDetailMap.get(response.getOrderNo());
...
//            merchantPrintService.printReceipt(merchantOrderListForPrint, 3);
    ...
    /**
     * 通过日期获取支付用户数量（去重）
     * @param merId 商户ID，0表示所有商户
     * @param date 日期，yyyy-MM-dd格式
     * @return Integer
     */
    @Override
    public Integer getPayUserNumByDate(Integer merId, String date) {
        QueryWrapper<Order> wrapper = new QueryWrapper<>();
        ...
        // 使用 Stream 去重
        return (int) orderList.stream().map(Order::getUid).distinct().count();
    }
    ...
    public Boolean updateInvoice(OrderInvoiceUpdateRequest request, SystemAdmin systemAdmin) {
        ...
        String oldExpressCode = invoice.getExpressCode(); // 保存原快递公司编码
        ...
        Boolean result = orderInvoiceService.updateById(invoice);
        
        // 同步订单快递信息到聚水潭（仅自营店且快递配送方式且快递公司发生变化）
        if (result && invoice.getDeliveryType().equals(OrderConstants.ORDER_DELIVERY_TYPE_EXPRESS) 
            && !Objects.equals(oldExpressCode, request.getExpressCode())) {
            try {
                Order order = getByOrderNo(invoice.getOrderNo());
                if (StrUtil.isNotBlank(order.getJstOrderId())) {
                    justuitanErpService.updateOrderLogisticsInJst(order.getJstOrderId(), request.getExpressCode());
                    logger.info("同步订单快递信息到聚水潭成功，订单号: {}, 快递公司: {}", order.getOrderNo(), request.getExpressCode());
                }
            } catch (Exception e) {
                logger.error("同步订单快递信息到聚水潭失败，发货单ID: {}", request.getId(), e);
                // 不影响主要业务流程，只记录错误日志
            }
        }
    ...
    private Boolean splitSendExpress(OrderSendRequest request, Order order, MerchantOrder merchantOrder, List<OrderDetail> orderDetailList) {
        ...
        // 聚水潭发货接口调用
        try {
            justuitanErpService.shipOrderToJst(order.getOrderNo(), express.getName(), electrResponse.getKuaidinum());
            logger.info("聚水潭拆单发货接口调用成功，订单号：{}", order.getOrderNo());
        } catch (Exception e) {
            logger.error("聚水潭拆单发货接口调用失败，订单号：{}，错误信息：{}", order.getOrderNo(), e.getMessage(), e);
        }
    ...
    private Boolean sendExpress(OrderSendRequest request, Order order, MerchantOrder merchantOrder) {
        ...
        // 为用户创建平台公告消息
        try {
            createShippingAnnouncementForUser(order, express.getName(), electrResponse.getKuaidinum());
        } catch (Exception ex) {
            logger.error("创建发货公告消息失败，订单号:{}, 错误信息:{}", order.getOrderNo(), ex.getMessage());
        }
        ...
        try {
            String expressName = "";
            if (StrUtil.isNotBlank(request.getExpressCode())) {
                Express expressInfo = expressService.getByCode(request.getExpressCode());
                if (expressInfo != null) {
                    expressName = expressInfo.getName();
                }
            }
            justuitanErpService.shipOrderToJst(order.getOrderNo(), expressName, request.getExpressNumber());
            logger.info("聚水潭发货接口调用成功，订单号：{}", order.getOrderNo());
        } catch (Exception e) {
            logger.error("聚水潭发货接口调用失败，订单号：{}，错误信息：{}", order.getOrderNo(), e.getMessage(), e);
        }
     ...
     * @return Integer
     */
    private Integer getCount(OrderSearchRequest request, String status, Integer merId) {
        Map<String, Object> map = CollUtil.newHashMap();
        ...
        if (StrUtil.isNotBlank(request.getBuyerNickname())) {
            map.put("buyerNickname", request.getBuyerNickname());
        }
        if (StrUtil.isNotBlank(request.getBuyerPhone())) {
            map.put("buyerPhone", request.getBuyerPhone());
        }
        if (StrUtil.isNotBlank(request.getPayType())) {
            map.put("payType", request.getPayType());
        }
        if (StrUtil.isNotBlank(request.getReceiverAddress())) {
            map.put("receiverAddress", request.getReceiverAddress());
        }
        if (StrUtil.isNotBlank(request.getExpressNo())) {
            map.put("expressNo", request.getExpressNo());
        }
        if (StrUtil.isNotBlank(request.getDeliveryMethod())) {
            map.put("deliveryMethod", request.getDeliveryMethod());
        }
        if (StrUtil.isNotBlank(request.getUserType())) {
            map.put("userType", request.getUserType());
        }
        if (ObjectUtil.isNotNull(request.getActivityType())) {
            map.put("activityType", request.getActivityType());
        }
     ...
     */
    @Override
    public OrderStatisticsResponse getMerchantOrderStatistics(Integer merId, Integer days) {
        ...
        String monthStart = DateUtil.format(DateUtil.beginOfMonth(new Date()), "yyyy-MM-dd");
        
        // 今日统计
        response.setTodayOrderCount(getOrderNumByDate(merId, today));
        response.setTodayOrderAmount(getPayOrderAmountByDate(merId, today));
        response.setTodayPaidOrderCount(getPayOrderNumByDate(today));
        response.setTodayPaidAmount(getPayOrderAmountByDate(merId, today));
        
        // 昨日统计  
        response.setYesterdayOrderCount(getOrderNumByDate(merId, yesterday));
        response.setYesterdayOrderAmount(getPayOrderAmountByDate(merId, yesterday));
        response.setYesterdayPaidOrderCount(getPayOrderNumByDate(yesterday));
        response.setYesterdayPaidAmount(getPayOrderAmountByDate(merId, yesterday));
        
        // 本月统计
        Integer monthOrderCount = 0;
        BigDecimal monthOrderAmount = BigDecimal.ZERO;
        Integer monthPaidOrderCount = 0;
        BigDecimal monthPaidAmount = BigDecimal.ZERO;
        
        // 计算本月每天的数据
        List<OrderStatisticsResponse.OrderTrendData> orderTrend = new ArrayList<>();
        List<OrderStatisticsResponse.OrderTrendData> amountTrend = new ArrayList<>();
        ...
        for (Date date = monthStartDate; !date.after(todayDate); date = DateUtil.offsetDay(date, 1)) {
            ...
            Integer dayOrderCount = getOrderNumByDate(merId, dateStr);
            BigDecimal dayOrderAmount = getPayOrderAmountByDate(merId, dateStr);
            Integer dayPaidCount = getPayOrderNumByDate(dateStr);
            BigDecimal dayPaidAmount = getPayOrderAmountByDate(merId, dateStr);
            
            monthOrderCount += dayOrderCount;
            monthOrderAmount = monthOrderAmount.add(dayOrderAmount);
            monthPaidOrderCount += dayPaidCount;
            monthPaidAmount = monthPaidAmount.add(dayPaidAmount);
            
            // 构建趋势数据
            OrderStatisticsResponse.OrderTrendData orderData = new OrderStatisticsResponse.OrderTrendData();
            orderData.setDate(dateStr);
            orderData.setCount(dayOrderCount);
            orderData.setValue(dayOrderAmount);
            orderTrend.add(orderData);
            
            OrderStatisticsResponse.OrderTrendData amountData = new OrderStatisticsResponse.OrderTrendData();
            amountData.setDate(dateStr);
            amountData.setCount(dayPaidCount);
            amountData.setValue(dayPaidAmount);
            amountTrend.add(amountData);
        }
        
        response.setMonthOrderCount(monthOrderCount);
        response.setMonthOrderAmount(monthOrderAmount);
        response.setMonthPaidOrderCount(monthPaidOrderCount);
        response.setMonthPaidAmount(monthPaidAmount);
        response.setOrderTrend(orderTrend);
        response.setAmountTrend(amountTrend);
     ...
     */
    @Override
    public OrderStatisticsResponse getPlatformOrderStatistics(Integer days) {
        return getMerchantOrderStatistics(null, days);
    }
     ...
     */
    @Override
    public OrderOverviewStatisticsResponse getOrderOverviewStatistics(OrderSearchRequest request, SystemAdmin systemAdmin) {
        Map<String, Object> searchMap = CollUtil.newHashMap();
        searchMap.put("merId", systemAdmin.getMerId());
        searchMap.put("status", request.getStatus());
        
        // 处理搜索条件
        if (StrUtil.isNotBlank(request.getDateLimit())) {
            DateLimitUtilVo dateLimit = CrmebDateUtil.getDateLimit(request.getDateLimit());
            searchMap.put("startTime", dateLimit.getStartTime());
            searchMap.put("endTime", dateLimit.getEndTime());
        }
        
        if (request.getSecondType() != null) {
            searchMap.put("secondType", request.getSecondType());
        }
        
        if (StrUtil.isNotBlank(request.getPayType())) {
            searchMap.put("payType", request.getPayType());
        }
        
        if (StrUtil.isNotBlank(request.getUserType())) {
            searchMap.put("userType", request.getUserType());
        }
        
        if (StrUtil.isNotBlank(request.getProductName())) {
            searchMap.put("productName", request.getProductName());
        }
        
        if (StrUtil.isNotBlank(request.getReceiverAddress())) {
            searchMap.put("receiverAddress", request.getReceiverAddress());
        }
        
        return dao.getOrderOverviewStatistics(searchMap);
    }

    @Override
    public void updateDiamond(String orderNo) {
        LambdaQueryWrapper<Order> lambda = new LambdaQueryWrapper<>();
        lambda.eq(Order::getOrderNo, orderNo);
        Order order = dao.selectOne(lambda);
        if (order == null) {
            throw new CrmebException("订单不存在");
        }
        //1元=10砖石
        BigDecimal diamond = order.getPayPrice().divide(new BigDecimal(10));
        order.setDiamond(diamond.intValue());
        //10砖石=1积分
        BigDecimal integral = diamond.multiply(new BigDecimal(10));
        order.setIntegral(integral.intValue());
     ...
     * 为用户创建发货平台公告消息
     */
    private void createShippingAnnouncementForUser(Order order, String expressName, String trackingNumber) {
        User user = userService.getById(order.getUid());
        MerchantAnnouncement announcement = new MerchantAnnouncement();
        announcement.setMerId(0); // 平台公告，商户ID为0
        announcement.setTitle("订单发货通知");
        announcement.setContent(String.format("亲爱的用户 %s，您的订单 %s 已通过 %s 发货，快递单号：%s，请注意查收！", 
            user.getNickname(), order.getOrderNo(), expressName, trackingNumber));
        announcement.setTargetType(1); // 目标用户类型：1-全部用户
        announcement.setStatus(1); // 公告状态：1-已发布
        announcement.setType(1); // 公告类型：1-系统公告
        ...
        merchantAnnouncementService.save(announcement);
    }

    @Override
    public List<Order> selectList(LambdaQueryWrapper<Order> lambda) {
        return dao.selectList(lambda);
    }

    @Override
    public BigDecimal getPayOrderTotalAmount(int merId) {
        QueryWrapper<Order> wrapper = new QueryWrapper<>();
        wrapper.select("pay_price");
        wrapper.eq("paid", 1);
        if (merId > 0) {
    ...
    @Override
    public LogisticsResultVo getLogisticsInfoByOrderNo(String orderNO) {
        List<OrderInvoiceResponse> orderInvoices = orderInvoiceService.findByOrderNo(orderNO);
        if (ObjectUtil.isNull(orderInvoices)) {
            throw new CrmebException(OrderResultCode.ORDER_INVOICE_NOT_EXIST);
        }
        OrderInvoiceResponse orderInvoiceResponse = orderInvoices.get(0);
        OrderInvoice orderInvoice = orderInvoiceService.getById(orderInvoiceResponse.getId());
        if (!orderInvoice.getDeliveryType().equals(OrderConstants.ORDER_DELIVERY_TYPE_EXPRESS)) {
            throw new CrmebException(OrderResultCode.ORDER_INVOICE_LOGISTICS_NOT_EXIST);
        }
        MerchantOrder merchantOrder = merchantOrderService.getOneByOrderNo(orderInvoice.getOrderNo());
