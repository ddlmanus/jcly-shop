...
import com.zbkj.common.IntegralAddRequest;
...
public class UserServiceImpl extends ServiceImpl<UserDao, User> implements UserService {
    ...
    public User registerPhone(String phone, Integer spreadUid) {
        ...
        if (execute) {
            try {
                // 发布用户创建事件，触发WebSocket推送
                dataChangeEventPublisher.publishUserCreated(user, UserConstants.REGISTER_TYPE_H5);
                logger.info("用户注册成功，已触发WebSocket推送: userId={}, phone={}", user.getId(), user.getPhone());
            } catch (Exception e) {
                logger.error("发布用户创建事件失败: userId={}", user.getId(), e);
                // 不影响用户注册流程，只记录错误
            }
        }
...
//        lqw.eq(User::getBirthday, birthday);
    ...
    @Override
    public Integer integralAdd(IntegralAddRequest request) {
         //判断手机号是否为空
        if (ObjectUtil.isNull(request.getPhone())) {
          //更当前登录人新增积分
            User info = this.getInfo();
            //判断积分是否大于1
            if (request.getIntegral() >= 1) {
                info.setIntegral(info.getIntegral() + request.getIntegral());
                this.dao.updateById( info);
                UserIntegralRecord integralRecord = new UserIntegralRecord();
                integralRecord.setUid(info.getId());
                integralRecord.setIntegral(request.getIntegral());
                integralRecord.setBalance(info.getIntegral());
                integralRecord.setTitle("游戏积分兑换");
                integralRecord.setMark("游戏积分兑换");
                integralRecord.setStatus(5);
                integralRecord.setType(1);
                integralRecord.setLinkId("0");
                integralRecord.setLinkType("system");
                integralRecord.setFrozenTime(0);
                integralRecord.setThawTime(0L);
                integralRecord.setCreateTime(new Date());
                integralRecord.setUpdateTime(new Date());
                userIntegralRecordService.save(integralRecord);
            }
            return info.getIntegral();
        }else{
            User user = this.getOne(new LambdaQueryWrapper<User>().eq(User::getPhone, request.getPhone()));
            if (ObjectUtil.isNull(user)) {
    ...
    @Override
    public UserAdminDetailResponse getAdminUserDetail(Integer id) {
