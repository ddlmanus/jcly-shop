...
//    }
     ...
     * @return 退款是否成功
     */
    private Boolean unionPayRefund(Order order, String outRefundNo, BigDecimal refundAmount) {
        try {
            logger.info("银联退款开始，订单号: {}, 退款单号: {}, 退款金额: {}", 
                       order.getOrderNo(), outRefundNo, refundAmount);
            
            // 调用银联退款接口
            Map<String, String> result = unionPayService.refundOrder(order.getOrderNo(), 
                                                                    order.getPayTime() != null ? 
                                                                    new SimpleDateFormat("yyyyMMddHHmmss").format(order.getPayTime()) : 
                                                                    DemoBase.getCurrentTime(), 
                                                                    refundAmount);
            
            if (result != null && "SUCCESS".equals(result.get("refundStatus"))) {
                logger.info("银联退款成功，订单号: {}, 退款单号: {}", order.getOrderNo(), outRefundNo);
                return true;
            } else {
                logger.warn("银联退款失败，订单号: {}, 失败原因: {}", 
                           order.getOrderNo(), result != null ? result.get("refundMessage") : "未知错误");
                return false;
            }
            
        } catch (Exception e) {
            logger.error("银联退款异常，订单号: {}, 退款单号: {}", order.getOrderNo(), outRefundNo, e);
            throw new RuntimeException("银联退款异常：" + e.getMessage(), e);
        }
    }
    ...
    public Boolean merchantDirectRefund(Order order, List<OrderDetail> orderDetailList) {
        ...
        Boolean unionPayIsRefund = false;
        if (order.getPayType().equals(PayConstants.PAY_TYPE_UNION_PAY) && totalRefundPrice.compareTo(BigDecimal.ZERO) > 0) {
            try {
                unionPayIsRefund = unionPayRefund(order, outRefundNo, totalRefundPrice);
            } catch (Exception e) {
                e.printStackTrace();
                throw new CrmebException("商家主动退款，银联申请退款失败！" + e.getMessage());
            }
        }
        ...
        Boolean finalUnionPayIsRefund = unionPayIsRefund;
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            if (order.getPayType().equals(PayConstants.PAY_TYPE_UNION_PAY) && finalTotalRefundPrice.compareTo(BigDecimal.ZERO) > 0 && finalUnionPayIsRefund) {
                refundOrderList.forEach(refundOrder -> {
                    refundOrder.setRefundStatus(OrderConstants.MERCHANT_REFUND_ORDER_STATUS_REFUND);
                });
            }
    ...
    private Boolean refundPrice(RefundOrder refundOrder, Order order) {
        ...
        Boolean unionPayIsRefund = false;
        if (order.getPayType().equals(PayConstants.PAY_TYPE_UNION_PAY) && refundPrice.compareTo(BigDecimal.ZERO) > 0) {
            try {
                unionPayIsRefund = unionPayRefund(order, refundOrder.getOutRefundNo(), refundPrice);
            } catch (Exception e) {
                e.printStackTrace();
                throw new CrmebException("银联申请退款失败！" + e.getMessage());
            }
        }
        Boolean aliPayIsRefund = false;
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            if (order.getPayType().equals(PayConstants.PAY_TYPE_UNION_PAY) && refundPrice.compareTo(BigDecimal.ZERO) > 0 && finalUnionPayIsRefund) {
                refundOrder.setRefundStatus(OrderConstants.MERCHANT_REFUND_ORDER_STATUS_REFUND);
            }
            updateById(refundOrder);
            return Boolean.TRUE;
        ...
        if (execute) {
            ...
            try {
                ...
                logger.info("成功更新聚水潭售后单状态为SUCCESS，订单号: {}, 售后单号: {}", order.getOrderNo(), refundOrder.getRefundOrderNo());
            } catch (Exception e) {
                logger.error("更新聚水潭售后单状态失败，订单号: {}, 售后单号: {}", order.getOrderNo(), refundOrder.getRefundOrderNo(), e);
            }
