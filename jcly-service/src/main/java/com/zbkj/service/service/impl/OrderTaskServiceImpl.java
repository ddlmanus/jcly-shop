...
public class OrderTaskServiceImpl implements OrderTaskService {
    ...
    private Boolean commonOrderCancel(Order order, String cancelMethod) {
        ...
        if (result != null && result) {
            try {
                String cancelType = cancelMethod.equals("auto") ? "系统取消" : "用户取消";
                String remark = cancelMethod.equals("auto") ? "到期未支付系统自动取消" : "用户主动取消订单";
                justuitanErpService.cancelOrderInJst(order.getOrderNo(), cancelType, remark);
            } catch (Exception e) {
    ...
    private Boolean rollbackStock(Order order) {
        ...
        for (OrderDetail orderDetail : orderDetailList) {
            ...
            // 同步聚水潭库存
            if (result && ObjectUtil.isNotNull(productAttrValue) && StrUtil.isNotBlank(productAttrValue.getJstSkuId())) {
                try {
                    justuitanErpService.syncInventory(orderDetail.getProductId(), orderDetail.getAttrValueId(), 
                            orderDetail.getPayNum(), "add");
                    logger.info("订单取消同步聚水潭库存成功，订单号：{}，商品ID：{}，SKU ID：{}，数量：{}", 
                            order.getOrderNo(), orderDetail.getProductId(), orderDetail.getAttrValueId(), orderDetail.getPayNum());
                } catch (Exception e) {
                    logger.error("订单取消同步聚水潭库存失败，订单号：{}，商品ID：{}，SKU ID：{}，错误信息：{}", 
                            order.getOrderNo(), orderDetail.getProductId(), orderDetail.getAttrValueId(), e.getMessage(), e);
                }
            }
        }
        return result;
    ...
    public void autoComplete() {
        ...
        Boolean execute = transactionTemplate.execute(e -> {
            ...
            orderNoList.forEach(orderNo -> {
                ...
                //更新订单的砖石
                orderService.updateDiamond(orderNo);
            });
            return Boolean.TRUE;
        });
        if (execute) {
