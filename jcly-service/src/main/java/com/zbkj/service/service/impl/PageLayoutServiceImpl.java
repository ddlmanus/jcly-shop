...
public class PageLayoutServiceImpl implements PageLayoutService {
    ...
    public PageLayoutIndexResponse index() {
        ...
        // 用户中心我的服务布局模式
        String userMenuLayoutMode = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_USER_MENU_LAYOUT_MODE);
        response.setUserMenuLayoutMode(userMenuLayoutMode);
        return response;
    }
    ...
    public Boolean userMenuSave(JSONObject jsonObject) {
        ...
        // 获取用户中心我的服务布局模式
        String userMenuLayoutMode = jsonObject.getString("userMenuLayoutMode");
        return transactionTemplate.execute(e -> {
            // 先删除历史数据
            ...
            systemGroupDataService.saveBatch(dataList, 100);
            // 保存用户中心我的服务布局模式
            if (StrUtil.isNotBlank(userMenuLayoutMode)) {
                systemConfigService.updateOrSaveValueByName(SysConfigConstants.CONFIG_USER_MENU_LAYOUT_MODE, userMenuLayoutMode);
            }
            return Boolean.TRUE;
        });
    }
    ...
    public PageLayoutBottomNavigationResponse getBottomNavigation() {
        ...
        String centerButtonIndex = systemConfigService.getValueByKey(SysConfigConstants.CONFIG_BOTTOM_NAVIGATION_CENTER_BUTTON_INDEX);
        String tabBarStyleMode=systemConfigService.getValueByKey(SysConfigConstants.CONFIG_BOTTOM_NAVIGATION_TAB_BAR_STYLE_MODE);
        // 是否自定义
        ...
        response.setCenterButtonIndex(centerButtonIndex);
        response.setTabBarStyleMode(tabBarStyleMode);
        return response;
    }
    ...
    public Boolean bottomNavigationSave(JSONObject jsonObject) {
        ...
        String centerButtonIndex = jsonObject.getString("centerButtonIndex");
        String tabBarStyleMode = jsonObject.getString("tabBarStyleMode");
        ...
        //保存 数据
        systemConfigService.updateOrSaveValueByName(SysConfigConstants.CONFIG_BOTTOM_NAVIGATION_CENTER_BUTTON_INDEX, centerButtonIndex);
        systemConfigService.updateOrSaveValueByName(SysConfigConstants.CONFIG_BOTTOM_NAVIGATION_TAB_BAR_STYLE_MODE, tabBarStyleMode);
    ...
    public Boolean splashAdConfigSave(SplashAdConfigVo configVo) {
        ...
        if (CollUtil.isNotEmpty(configVo.getAdList())) {
            configList = configVo.getAdList().stream().map(data -> {
                ...
                groupConfig.setMiniprogramAppId(data.getMiniprogramAppId());
                groupConfig.setMiniprogramPath(data.getMiniprogramPath());
                groupConfig.setMiniprogramParams(data.getMiniprogramParams());
                groupConfig.setIsMiniprogram(data.getIsMiniprogram());
