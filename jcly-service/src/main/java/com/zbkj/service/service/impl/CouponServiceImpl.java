...
public class CouponServiceImpl extends ServiceImpl<CouponDao, Coupon> implements CouponService {
    ...
    public Boolean create(CouponRequest request) {
        ...
        return transactionTemplate.execute(e -> {
            ...
            try {
                knowledgeMarkdownService.generateAndUploadCouponMarkdown(
                        coupon.getId(), knowledgeId, coupon.getMerId());
                log.info("优惠券知识库上传成功，优惠券ID: {}", coupon.getId());
            } catch (Exception ex) {
                log.error("优惠券知识库上传失败，优惠券ID: {}", coupon.getId(), ex);
            }
...
//                    }).start();
//                } else {
//                    log.warn("未配置Coze知识库ID，跳过优惠券知识库上传");
//                }
//            } catch (Exception ex) {
//                log.error("启动优惠券知识库上传任务失败", ex);
//            }
//
            return Boolean.TRUE;
        });
