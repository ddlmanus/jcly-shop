...
public class ProductReplyServiceImpl extends ServiceImpl<ProductReplyDao, ProductReply>
     ...
     * @param type 评价类型
     * @return Map<商品ID, 评论数量>
     */
    @Override
    public Map<Integer, Integer> batchGetReplyCountByProductIds(List<Integer> productIds, String type) {
        if (CollUtil.isEmpty(productIds)) {
            return CollUtil.newHashMap();
        }
        
        try {
            List<Map<String, Object>> resultList = dao.batchGetReplyCountByProductIds(productIds, type);
            Map<Integer, Integer> resultMap = CollUtil.newHashMap();
            
            // 将查询结果转换为 Map<Integer, Integer>
            for (Map<String, Object> item : resultList) {
                Integer productId = (Integer) item.get("key");
                Long count = (Long) item.get("value");
                resultMap.put(productId, count.intValue());
            }
            
            return resultMap;
        } catch (Exception e) {
            log.error("批量查询商品评论统计失败", e);
            return CollUtil.newHashMap();
        }
    }
}
