...
public class CityRegionServiceImpl extends ServiceImpl<CityRegionDao, CityRegion> implements CityRegionService {
    ...
    public List<CityResponse> getCityRegionList(CitySearchRequest request) {
        ...
        if(request.getRegionName() != null){
            lqw.like(CityRegion::getRegionName, request.getRegionName());
        }
    ...
    public List<CityVo> getProvinceList() {
        List<CityRegion> provinceList = findProvinceList();
        return provinceList.stream().map(region -> {
            CityVo cityVo = new CityVo();
            cityVo.setRegionId(region.getRegionId());
            cityVo.setParentId(region.getParentId());
            cityVo.setRegionName(region.getRegionName());
            cityVo.setRegionType(region.getRegionType());
            return cityVo;
        }).collect(Collectors.toList());
    }
     ...
     */
    @Override
    public List<CityVo> getCityListByProvinceId(Integer provinceId) {
        List<CityRegion> cityList = findCityList(provinceId);
        return cityList.stream().map(region -> {
            CityVo cityVo = new CityVo();
            cityVo.setRegionId(region.getRegionId());
            cityVo.setParentId(region.getParentId());
            cityVo.setRegionName(region.getRegionName());
     ...
     */
    @Override
    public List<CityVo> getDistrictListByCityId(Integer cityId) {
        List<CityRegion> districtList = findDistrictList(cityId);
        return districtList.stream().map(region -> {
            CityVo cityVo = new CityVo();
            cityVo.setRegionId(region.getRegionId());
            cityVo.setParentId(region.getParentId());
            cityVo.setRegionName(region.getRegionName());
     ...
     */
    @Override
    public List<CityVo> getStreetListByDistrictId(Integer districtId) {
        List<CityRegion> streetList = findStreetList(districtId);
        return streetList.stream().map(region -> {
            CityVo cityVo = new CityVo();
            cityVo.setRegionId(region.getRegionId());
            cityVo.setParentId(region.getParentId());
            cityVo.setRegionName(region.getRegionName());
