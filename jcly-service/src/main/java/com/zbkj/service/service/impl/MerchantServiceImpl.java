...
public class MerchantServiceImpl extends ServiceImpl<MerchantDao, Merchant> implements MerchantService {
    ...
    private Boolean commonAdd(MerchantAddRequest request, String createType, Integer createId) {
        ...
        PageDiy pageDiy1 = pageDiyService.getById(195);
        BeanUtils.copyProperties(pageDiy1, pageDiy);
        pageDiy.setId(null);
    ...
    public Boolean open(Integer id) {
        ...
        merchant.setUpdateTime(DateUtil.date());
        Boolean result = dao.updateById(merchant) > 0;
        
        if (result) {
            try {
                // 发布商户更新事件，触发WebSocket推送
                Map<String, Object> changeFields = new HashMap<>();
                changeFields.put("isSwitch", true);
                changeFields.put("isForceShutdown", false);
                dataChangeEventPublisher.publishMerchantUpdated(merchant, changeFields);
                logger.info("商户开启成功，已触发WebSocket推送: merchantId={}, merchantName={}", merchant.getId(), merchant.getName());
            } catch (Exception e) {
                logger.error("发布商户更新事件失败: merchantId={}", merchant.getId(), e);
                // 不影响商户开启流程，只记录错误
            }
        }
...
//        if (info.getId() > 0) {
//            response.setIsCollect(userMerchantCollectService.isCollect(info.getId(), merchant.getId()));
//        } else {
//            response.setIsCollect(false);
//        }
        return response;
    ...
    @Override
    public boolean checkMerchantExists(Integer merId) {
        return ObjectUtil.isNotNull(getById(merId));
    }

    @Override
    public List<Merchant> getByIdList(List<Integer> merIdList) {
        LambdaQueryWrapper<Merchant> lqw = Wrappers.lambdaQuery();
        return dao.selectList(lqw.in(Merchant::getId, merIdList));
    }
